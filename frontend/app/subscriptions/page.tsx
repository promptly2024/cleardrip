...
export default function SubscriptionsSection() {
  ...
  return (
    <section
      className="py-12 sm:py-16 lg:py-24 px-4"
      style={{ background: "linear-gradient(to bottom right, var(--white-50), var(--blue-50), var(--white-100))" }}
      aria-busy={loading}
    >
        ...
        <div className="flex flex-col xl:flex-row gap-8 lg:gap-12 items-stretch">
          ...
          <div className="xl:w-7/12 order-1 xl:order-2">
            {loading ? (
              ...
              <div className="text-center py-20 text-xl" style={{ color: "var(--blue-800)" }}>
...
const PricingCard = React.memo(function PricingCard({
  ...
  return (
    <div
      className={`pricing-card relative bg-white rounded-3xl p-8 lg:p-10 shadow-lg hover:shadow-2xl transition-all duration-500 border-2 group ${plan.popular ? "ring-4 scale-105 lg:scale-110" : "hover:border-blue-300"
        }`}
    ...
    >
      ...
      {plan.popular && (
        <div className="absolute -top-4 left-1/2 transform -translate-x-1/2" title="Most popular plan">
          <div
            ...
            style={{ background: "linear-gradient(to right, var(--blue-600), var(--blue-700))" }}
          >
      ...
      <div className="text-center mb-8">
        <div className="flex items-baseline justify-center gap-2 mb-2">
          <span className="text-3xl lg:text-4xl font-bold" style={{ color: "var(--blue-900)" }}>
            {formattedPrice}
          </span>
          <span className="text-lg" style={{ color: "var(--blue-700)" }}>
            / {durationLabel}
          </span>
        </div>
          ...
          {plan.features.map((feature, idx) => (
            <li key={idx} className="flex items-start gap-3">
              <div
                ...
                style={{ backgroundColor: plan.popular ? "var(--blue-100)" : "var(--white-200)" }}
              >
                <Check className="w-3 h-3" style={{ color: plan.popular ? "var(--blue-600)" : "var(--blue-800)" }} />
              </div>
              <span className="leading-relaxed" style={{ color: "var(--blue-900)" }}>
                {feature}
              </span>
            </li>
          ))}
...
// Add CSS for animations (kept as before but will be injected client-side)
const styles = `
@keyframes fadeInUp {
  from {
    opacity: 0;
...
.subscribe-btn[disabled] {
  opacity: 0.7;
  cursor: not-allowed;
  filter: grayscale(.05) brightness(.95);
  transform: none;
}
