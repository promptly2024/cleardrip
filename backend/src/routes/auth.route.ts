import { AdminSignInHandler, AdminGetAllListHandler, CreateAdminUserHandler, DeleteStaff, AdminDashboardOverviewHandler } from "@/controllers/auth/auth.admin.controller"
import { meHandler } from "@/controllers/auth/auth.me.controller"
import { signinHandler, signoutHandler, signupHandler, forgotPasswordHandler, resetPasswordHandler, updateProfileHandler, updateFCMTokenHandler, verifyResetTokenHandler } from "@/controllers/auth/auth.user.controller"
...
export default async function authRoutes(fastify: FastifyInstance) {
    ...
    fastify.get("/admin/dashboard/stats", { preHandler: requireRole(["SUPER_ADMIN"]) }, AdminDashboardOverviewHandler)
}